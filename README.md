...
- 表格首行/合并单元格内表题识别。
- 正文表格引用识别。
- 高置信度编号匹配，包括原始统计表编号和题注重排后的新编号。
- 重复局部编号隔离：多个章节都出现 `见表1` / `表1 xxx` 时，优先匹配引用段落后的最近同编号表，避免串到其他章节。
- 语义邻近纠偏：对 `见表 x` / `见表格 x` 会结合正文整句和附近表题关键词匹配，例如正文包含 `重要方案偏离` 时优先匹配附近的 `重要方案偏离情况` 表题。
- 中置信度段落邻近匹配，例如 `统计列表 16.2.2` 推荐匹配到同段落后的最近表格。
- 题注字段写入：`SEQ 表 \* ARABIC`。
- 书签写入：`_CSR_Table_001`。
