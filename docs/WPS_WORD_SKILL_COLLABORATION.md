...
其中“表格3”是真实交叉引用字段，字体颜色为蓝色（`#0000FF`），在正文中可视化区分。

#### 交叉引用字体颜色规则

创建交叉引用时，引用文本（含前缀和编号，如“表格3”、“图 1”）必须设置为蓝色字体（`#0000FF`），以便用户在正文中快速识别哪些内容是可点击跳转的交叉引用。

OOXML 实现方式：在引用 run 中添加 `<w:rPr><w:color w:val="0000FF"/></w:rPr>`。
