...
class DemoHandler(BaseHTTPRequestHandler):
    server_version = "CSRDocAgentDemo/0.2"
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            strategy = query.get("strategy", ["continuous"])[0]

            if parsed.path == "/api/analyze":
                plan = analyze_docx(docx_bytes, strategy=strategy)
                self._send_json(plan)
                return

            if parsed.path == "/api/process":
                filename = query.get("filename", ["processed.docx"])[0]
                confirmed_raw = query.get("confirmed", [""])[0]
                confirmed_refs = (
                    set(confirmed_raw.split(",")) if confirmed_raw else set()
                )
                output_bytes, audit = process_docx(
                    docx_bytes,
                    strategy=strategy,
